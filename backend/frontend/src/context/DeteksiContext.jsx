...
export function DeteksiProvider({ children }) {
  ...
  // Fetch detections
  const fetchDetections = useCallback(async (page = currentPage) => {
    setLoading(true)
    try {
      const response = await apiRequest(`${API_ENDPOINTS.DETECTION_LIST}?page=${page}&limit=${itemsPerPage}`)
      if (response.success) {
  ...
  useEffect(() => {
    ...
    socket.on('detection-status-changed', (data) => {
      ...
      console.log('🔍 Current trackingId:', currentTrackingId)
      console.log('🔍 Received trackingId:', data.trackingId)
      
      if (data.stage === 'completed') {
        console.log('✅ Detection completed! Refreshing data...')
        fetchDetections()
        
        setSuccessMessage(`Deteksi selesai! Total kendaraan: ${data.countingData?.totalCounted || data.totalVehicles || 0}`)
        setTimeout(() => setSuccessMessage(''), 5000)
        
        // Update UI - always try to update
        setUploading(false)
        setRealTimeProgress(null)
        setUploadProgress('')
        
        if (data.outputVideoUrl) {
          console.log('🎬 Setting video preview:', data.outputVideoUrl)
